 ...
 * 2. Parent has `.planning/config.json` with `multiRepo: true` (older format)
...
/**
 * Remove stale thrunt-* temp files/dirs older than maxAgeMs (default: 5 minutes).
 * Runs opportunistically before each new temp file write to prevent unbounded accumulation.
 * @param {string} prefix - filename prefix to match (e.g., 'thrunt-')
 * @param {object} opts
 ...
 */
function reapStaleTempFiles(prefix = 'thrunt-', { maxAgeMs = 5 * 60 * 1000, dirsOnly = false } = {}) {
  try {
...
function output(result, raw, rawValue) {
  ...
  if (raw && rawValue !== undefined) {
    ...
    if (json.length > 50000) {
      reapStaleTempFiles();
      const tmpPath = path.join(require('os').tmpdir(), `thrunt-${Date.now()}.json`);
...
function loadConfig(cwd) {
  ...
  try {
    ...
    // Migrate legacy discuss mode to the canonical enum.
    if (parsed.workflow && typeof parsed.workflow === 'object' && parsed.workflow.discuss_mode === 'standard') {
      parsed.workflow.discuss_mode = 'discuss';
      configDirty = true;
    }

    // Auto-detect and sync sub_repos: scan for child directories with .git

    // Migrate older "multiRepo: true" boolean → sub_repos array
    ...
    return {
      ...
      plan_check: get('plan_check', { section: 'workflow', field: 'plan_check' }) ?? defaults.plan_check,
      validator: get('validator', { section: 'workflow', field: 'validator' }) ?? defaults.validator,
 ...
 * Applied at write points so THRUNT-generated .planning/ files are IDE-friendly.
 *
 * Rules enforced:
...
function planningPaths(cwd, wsOrOpts) {
  ...
  return {
    ...
    huntmap: path.join(base, 'HUNTMAP.md'),
    hypotheses: path.join(base, 'HYPOTHESES.md'),
    successCriteria: path.join(base, 'SUCCESS_CRITERIA.md'),
    findings: path.join(base, 'FINDINGS.md'),
    evidenceReview: path.join(base, 'EVIDENCE_REVIEW.md'),
...
function getMissionDocInfo(cwd) {
  const paths = planningPaths(cwd);
  return {
    exists: fs.existsSync(paths.mission),
    path: paths.mission,
    source: path.basename(paths.mission),
    mission: paths.mission,
  };
}

function getHuntmapDocInfo(cwd, ws) {
  const paths = planningPaths(cwd, ws);
  return {
    exists: fs.existsSync(paths.huntmap),
    path: paths.huntmap,
    source: path.basename(paths.huntmap),
    huntmap: paths.huntmap,
  };
}
...
function searchPhaseInDir(baseDir, relBase, normalized) {
  try {
    ...
    const { plans: unsortedPlans, summaries: unsortedSummaries, hasResearch, hasContext, hasFindings, hasReviews } = getPhaseFileStats(phaseDir);
...
// ─── Huntmap milestone scoping ───────────────────────────────────────────────

/**
 * Strip published milestone content wrapped in <details> blocks.
 * Used to isolate current milestone phases when searching HUNTMAP.md
...
/**
 * Extract the current milestone section from HUNTMAP.md by positive lookup.
 *
 ...
 * - Version can't be found in HUNTMAP.md
 *
 * @param {string} content - Full HUNTMAP.md content
...
function extractCurrentMilestone(content, cwd) {
  ...
  // 2. Fallback: derive version from getMilestoneInfo pattern in HUNTMAP.md itself
  if (!version) {
  ...
  // Match headings like: ## Huntmap v3.0: Name, ## v3.0 Name, etc.
  ...
  // Milestone headings look like: ## v2.0, ## Huntmap v2.0, ## ✅ v1.0, etc.
  ...
  // but strip any <details> blocks in it (these are definitely published)
...
/**
 * Replace a pattern only in the current milestone section of HUNTMAP.md
...
// ─── Huntmap & model utilities ────────────────────────────────────────────────

function getHuntmapPhaseInternal(cwd, phaseNum) {
  if (!phaseNum) return null;
  const huntmapDoc = getHuntmapDocInfo(cwd);
  if (!huntmapDoc.exists) return null;

  try {
    const content = extractCurrentMilestone(fs.readFileSync(huntmapDoc.path, 'utf-8'), cwd);
...
/**
 * Resolve the agents directory from the THRUNT install location.
 * thrunt-tools.cjs lives at <configDir>/thrunt-god/bin/thrunt-tools.cjs,
 * so agents/ is at <configDir>/agents/.
 *
 * @returns {string} Absolute path to the agents directory
 */
function getAgentsDir() {
  // __dirname is thrunt-god/bin/lib/ → go up 3 levels to configDir
  return path.join(__dirname, '..', '..', '..', 'agents');
}

/**
 * Check which THRUNT agents are installed on disk.
...
function getMilestoneInfo(cwd) {
  try {
    const huntmapDoc = getHuntmapDocInfo(cwd);
    const huntmap = fs.readFileSync(huntmapDoc.path, 'utf-8');

    // First: check for list-format huntmaps using 🚧 (in-progress) marker
    // e.g. "- 🚧 **v2.1 Belgium** — Phases 24-28 (in progress)"
    // e.g. "- 🚧 **v1.2.1 Tech Debt** — Phases 1-8 (in progress)"
    const inProgressMatch = huntmap.match(/🚧\s*\*\*v(\d+(?:\.\d+)+)\s+([^*]+)\*\*/);
    if (inProgressMatch) {
      return {
        version: 'v' + inProgressMatch[1],
    ...
    // Second: heading-format huntmaps — strip published milestones in <details> blocks
    const cleaned = stripShippedMilestones(huntmap);
    ...
    const huntmapMatch = cleaned.match(/^#\s*Huntmap:\s*(.+)$/mi);
    if (huntmapMatch) {
      return {
        version: 'v1.0',
        name: huntmapMatch[1].trim(),
      };
    }
 ...
 * to the current milestone based on HUNTMAP.md phase headings.
...
function getMilestonePhaseFilter(cwd) {
  ...
  try {
    const huntmapDoc = getHuntmapDocInfo(cwd);
    const huntmap = extractCurrentMilestone(fs.readFileSync(huntmapDoc.path, 'utf-8'), cwd);
    // Match both numeric phases (Phase 1:) and custom IDs (Phase PROJ-42:)
    const phasePattern = /#{2,4}\s*Phase\s+([\w][\w.-]*)\s*:/gi;
    let m;
    while ((m = phasePattern.exec(huntmap)) !== null) {
 ...
 * research/context/findings files.
 */
function getPhaseFileStats(phaseDir) {
  ...
  return {
    ...
    hasFindings: files.some(f => f.endsWith('-FINDINGS.md') || f === 'FINDINGS.md'),
